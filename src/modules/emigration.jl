...
"""
    compute_prob_emigration(year_index, age, sex, emigration)
...
"""
function compute_prob_emigration(year_index::Integer, age::Integer, sex::Bool,
    emigration::Emigration
)
    if age == 0
        return false
    else
        return rand(Bernoulli(emigration.table[year_index][min(age, 100), sex + 3]))
    end
end
